...
frappe.ui.form.on('Payment Entry', {
    ...
    setup: function (frm) {
        ...
        frm.set_query("paid_from", function () {
            if (frm.doc.mode_of_payment != 'Cash' && frm.doc.mode_of_payment != undefined) {
                account_types = 'Bank'
            }
            else {
    ...
    hide_unhide_fields: function (frm) {
        ...
        if (frm.doc.paid_from_account_currency != null && frm.doc.payment_type != 'Internal Transfer'){
            frm.set_value("paid_to_account_currency", frm.doc.paid_from_account_currency)
        }
         else if (frm.doc.paid_to_account_currency != null && frm.doc.payment_type != 'Internal Transfer'){
            frm.set_value("paid_from_account_currency", frm.doc.paid_to_account_currency)
        }

        if(frm.doc.party_type == null && frm.doc.payment_type != 'Internal Transfer'){
        ...
        if (frm.doc.payment_type == 'Internal Transfer'){
            frm.toggle_display("deductions_or_loss_section", false);
        }
        ...
        if (frm.doc.mode_of_payment == 'Cheque') {
                frm.toggle_reqd(["reference_no", "reference_date"], true);
            }
    },
            ...
            frappe.call({
                ...
                callback: function (r, rt) {
                    if (r.message) {
                        ...
                        if ( frm.doc.mode_of_payment == 'Cheque' && frm.doc.payment_type == "Receive" && currency_field == "paid_to_account_currency") {
                            frm.toggle_reqd(["reference_no", "reference_date"],
                                (r.message['account_type'] == "Bank" ? 1 : 0));
                            if (!frm.doc.received_amount && frm.doc.paid_amount)
                                frm.events.paid_amount(frm);
                        } else if ( frm.doc.mode_of_payment == 'Cheque' && frm.doc.payment_type == "Pay" && currency_field == "paid_from_account_currency") {
                            frm.toggle_reqd(["reference_no", "reference_date"],
                                (r.message['account_type'] == "Bank" ? 1 : 0));
                            ...
                            frm.toggle_display(["reference_no", "reference_date"], true);
                            frm.toggle_reqd(["reference_no", "reference_date"], false );
                        }
                        else{
                            frm.toggle_reqd(["reference_no", "reference_date"], false );
    ...
    set_difference_amount: function (frm) {
        ...
        // new for unallocated in case of payment of expenses
        if (frm.doc.payment_type != 'Internal Transfer') {
            if (frm.doc.party_type) {
                frm.set_value("difference_amount", difference_amount - total_deductions);
            }
            else {
                frm.set_value('unallocated_amount', frm.doc.paid_amount - frm.doc.total_allocated_amount - total_deductions);
                frm.set_value('difference_amount', frm.doc.total_allocated_amount - frm.doc.unallocated_amount);
            }
        }
        frm.events.hide_unhide_fields(frm);
    },
