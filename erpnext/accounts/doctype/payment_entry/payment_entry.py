...
class PaymentEntry(AccountsController):
    ...
    def validate(self):
        ...
        validate_frozen_account(self.paid_from, None)
        validate_frozen_account(self.paid_to, None)
    ...
    def check_drawn_on(self):
        if self.mode_of_payment == 'Cheque' and self.drawn_on == None:
            frappe.throw(_("Drawn On Bank"))
    ...
    def ship_to_address(self):
        ship_add = get_customer_ship_address(self.party, self.party_type)
        frappe.db.set(self, 'shipping_address', ship_add)
        frappe.db.commit()
    ...
    def set_difference_amount(self):
        ...
        if self.payment_type != 'Internal Transfer':
            if self.party_type:
                ...
                self.unallocated_amount = flt(self.paid_amount - self.total_allocated_amount - total_deductions,
                                         self.precision("difference_amount"))

                self.difference_amount = flt(self.total_allocated_amount - self.unallocated_amount,
                                         self.precision("difference_amount"))
...
def get_company_pos_address():
    ...
    if (len(add) > 0):
        add = add[0]
        pos_add = """{0} {1}<br/>{2}, {3}<br/>Phone: {4}<br/>Email: {5}""".format(add.address_line1,
                                                                                  add.address_line2,
...
def get_customer_ship_address(ship_to, party_type):
    pos_add = ""
...
WHERE dl.link_doctype = '{0}'
AND a.address_type='Billing'
AND a.is_primary_address = 1
AND dl.link_name = '{1}'""".format(party_type, ship_to)
    add = frappe.db.sql(sql, as_dict=1)
    if (len(add) > 0):
        add = add[0]
