...
class SalesInvoice(SellingController):
    ...
    def validate_purchase_invoice(self):
        if (self.transaction_type_name == 'Profit Margin' or self.transaction_type_name == 'RCM Supplies') and self.purchase_invoice == None:
            ...
            for allitems in self.items:
                for taxRow in self.taxes:
                    ...
                    if self.docstatus == 0:
                        ...
                        frappe.db.set_value("Sales Invoice Item", allitems.name, "tax_rate", tax_rate, update_modified=False)
                        frappe.db.set_value("Sales Invoice Item", allitems.name, "tax_amount", tax_amount, update_modified=False)
                        frappe.db.set_value("Sales Invoice Item", allitems.name, "item_with_tax_amount", item_with_tax_amount, update_modified=False)
        except Exception, ex:
            print ex
    ...
    def update_sales_tax_and_charges(self):
        ...
        sql = """ UPDATE `tabSales Taxes and Charges` SET item_wise_tax_detail = '{0}' WHERE parent = '{1}'  """.format(data_dict, self.name)
        frappe.db.sql(sql)
        frappe.db.commit()
