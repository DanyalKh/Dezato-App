...
def save_entries(gl_map, adv_adj, update_outstanding, from_repost=False):
    ...
    delete_gl2_entries(gl_map[0].voucher_type, gl_map[0].voucher_no)
    for entry in gl_map:
...
def make_entry(args, adv_adj, update_outstanding, from_repost=False):
    docstatus = frappe.get_doc(args.voucher_type, args.voucher_no).docstatus
    if docstatus == 0:
        args.update({"doctype": "GL Entry 2"})
    else:
...
def round_off_debit_credit(gl_map):
    precision = get_field_precision(frappe.get_meta("GL Entry").get_field("debit"),
                                    currency=frappe.db.get_value("Company", gl_map[0].company, "default_currency",
                                                                 cache=True))
...
def delete_gl2_entries(voucher_type=None, voucher_no=None, adv_adj=False, update_outstanding="Yes"):
    from erpnext.accounts.doctype.gl_entry.gl_entry import validate_balance_type, \
        check_freezing_date, update_outstanding_amt, validate_frozen_account
    ...
    frappe.db.sql("""delete from `tabGL Entry 2` where voucher_type=%s and voucher_no=%s""",
