...
def set_gl_entries2_by_account(company, from_date, to_date, root_lft, root_rgt, filters, gl_entries_by_account,
...
def set_gl_entries_by_account(company, from_date, to_date, root_lft, root_rgt, filters, gl_entries_by_account,
    ...
    if filters != None:
        if filters.get("transaction_type"):
            unapproved_transactions = " UNION select posting_date, account, debit, credit, is_opening, fiscal_year from `tabGL Entry 2` where company=%(company)s {additional_conditions} and posting_date <= %(to_date)s and account in (select name from `tabAccount` where lft >= %(lft)s and rgt <= %(rgt)s) ".format(additional_conditions=additional_conditions)
    ...
    gl_entries = frappe.db.sql("""select posting_date, account, debit, credit, is_opening, fiscal_year from `tabGL Entry`
		...
		and account in (select name from `tabAccount`
			where lft >= %(lft)s and rgt <= %(rgt)s)
        {unapproved_transactions}
		order by account, posting_date""".format(additional_conditions=additional_conditions, unapproved_transactions = unapproved_transactions),
                               {
                                   "company": company,
                                   "from_date": from_date,
                                   "to_date": to_date,
