...
def get_columns(filters):
    columns = [
        ...
        {"label": _("Debit"), "fieldname": "debit", "fieldtype": "Float", "width": 100, 'precision':2},
        {"label": _("Credit"), "fieldname": "credit", "fieldtype": "Float", "width": 100, 'precision': 2},
    ]
    ...
    columns += [
        # _("Balance") + ":Float:120",
         {"label": _("Balance"), "fieldname": "balance", "fieldtype": "Float", "width": 100, 'precision': 2},
        _("Debit / Credit") + "::120",
        _("Voucher Type") + "::120", _("Voucher No") + ":Dynamic Link/" + _("Voucher Type") + ":160",
        _("Document Status") + ":Data:150",
...
def get_result(filters, account_details):
    if filters.get("transaction_type"):
        gl_entries = get_gl_entries2(filters)
    else:
...
def get_gl_entries2(filters):
    ...
    gl_entries = frappe.db.sql("""
		...
		select
			...
			remarks, against, is_opening {select_fields}
		from `tabGL Entry 2`
		where company=%(company)s {conditions2}
		{group_by_condition}		

		order by posting_date, account""" \
                               .format(select_fields=select_fields, conditions=get_conditions(filters),
                                       conditions2=get_conditions(filters, "GL Entry 2"),
                                       group_by_condition=group_by_condition), filters, as_dict=1)
    for gl in gl_entries:
        if gl.voucher_type in ['Payment Entry', 'Sales Invoice', 'Purchase Invoice', 'Journal Entry']:
            try:
                document_status = frappe.get_doc(gl.voucher_type, gl.voucher_no).workflow_state
                gl['document_status'] = document_status
            except Exception as e:
                print e
    return gl_entries


def get_conditions(filters, p_doctype="GL Entry"):
    conditions = []
    if filters.get("account"):
    ...
    match_conditions = build_match_conditions(p_doctype)
...
def get_result_as_list(data, filters):
    ...
    Balance = 0
    for d in data:
        if d.get("debit") is None:
            d['debit'] = 0
        elif d.get("credit") is None:
            d['credit'] = 0
        try:
            value = d['credit']
        except KeyError:
        ...
        Balance = float(Balance) + float(d['debit']) - float(d['credit'])
        if Balance == 0:
            dr_cr = ""
        elif Balance > 0:
            dr_cr = "Dr"
        ...
        if d.get("account") == "'Totals'" or d.get("account") == "'Closing (Opening + Totals)'":
            Balance = 0
            dr_cr = ''
        ...
        row = [d.get("posting_date"), d.get("account"), flt(d.get("debit"), 2), flt(d.get("credit"), 2),
               flt(abs(Balance), 2), dr_cr]

        if filters.get("show_in_account_currency"):
            row += [d.get("debit_in_account_currency"), d.get("credit_in_account_currency")]

        row += [d.get("voucher_type"), d.get("voucher_no"), document_status, d.get("against"),
