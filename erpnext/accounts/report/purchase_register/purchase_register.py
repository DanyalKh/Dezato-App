...
def _execute(filters=None, additional_table_columns=None, additional_query_columns=None):
	...
	for inv in invoice_list:
		...
		row = [inv.name, inv.posting_date, inv.due_date, inv.supplier, inv.supplier_name]
...
def get_columns(invoice_list, additional_table_columns):
	...
	columns = [
		_("Invoice") + ":Link/Purchase Invoice:120", _("Posting Date") + ":Date:100",_("Due Date") + ":Date:100",
	...
	columns += [
		...
		_("Mode of Payment") + ":Link/Mode of Payment:120", _("Project") + ":Link/Project:80",
		_("Bill No") + "::120", _("Bill Date") + ":Date:80", _("Remarks") + "::150",
		_("Purchase Order") + ":Link/Purchase Order:120",
		_("Purchase Receipt") + ":Link/Purchase Receipt:120",
		{
			"fieldname": "currency",
			"label": _("Currency"),
			"fieldtype": "Data",
	...
	expense_columns = [(account + ":Currency/currency:180") for account in expense_accounts]
	for account in tax_accounts:
		if account not in expense_accounts:
			tax_columns.append(account + ":Currency/currency:180")

	columns = columns + expense_columns + [_("Net Total") + ":Currency/currency:120"] + tax_columns + \
		[_("Total Tax") + ":Currency/currency:120", _("Grand Total") + ":Currency/currency:120",
			_("Rounded Total") + ":Currency/currency:120", _("Outstanding Amount") + ":Currency/currency:150"]
...
def get_invoices(filters, additional_query_columns):
	...
	return frappe.db.sql("""
		select
			name, posting_date,due_date, credit_to, supplier, supplier_name, bill_no, bill_date,
