...
// Get present and absent details
//----------------------------------------------------------------------
cur_frm.cscript.end_date = function(doc, dt, dn){
    debugger;
    if (doc.employee && doc.start_date && doc.end_date) {
            get_attendance_details(doc, dt, dn);
    	}
};

cur_frm.cscript.absents = function(doc, dt, dn){
		 debugger;
		 if (doc.employee && doc.start_date && doc.end_date) {
		 	calculate_all(doc, dt, dn);
...
var get_attendance_details = function (doc, dt, dn) {
    frappe.call({
				method: 'get_attendance_details',
				doc: locals[dt][dn],
				args: {
			...
			callback: function (r) {
                cur_frm.set_value('payment_days', r.message[0] );
                cur_frm.set_value('absents' , r.message[1] );
			}
		})
}
