...
class SalarySlip(TransactionBase):
    ...
    def get_attendance_details(self, start_date, end_date):
        presents = frappe.db.sql("""select count(*) as no_of_presents from tabAttendance att
                        where att.employee = %s
                        and att.attendance_date BETWEEN %s AND %s
                        and att.`status` not in ('Absent')
                        and att.docstatus = 1 """,
                                 (self.employee, start_date, end_date), as_dict=1)
        if not presents:
            presents = 0
        else:
            ...
            absents = working_days - presents[0].no_of_presents
            presents = presents[0].no_of_presents
            return presents, absents, working_days
    ...
    def get_leave_details(self, joining_date=None, relieving_date=None, lwp=None):
        ...
        # absents = self.get_absents_for_employee(self.start_date, self.end_date)
        payment_days = flt(self.get_payment_days(joining_date, relieving_date)) - flt(lwp)
        self.payment_days = payment_days > 0 and payment_days or 0

    def get_absents_for_employee(self, start_date, end_date):
        absents = frappe.db.sql("""select count(*) from tabAttendance att
	        where att.employee = %s
	        and att.attendance_date BETWEEN %s AND %s
