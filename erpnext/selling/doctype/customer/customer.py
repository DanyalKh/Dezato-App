...
class Customer(TransactionBase):
    ...
    def item_check(self):
        ...
        import re
        regex = r"^[A-z0-9= .&]+$"
        if not re.search(regex, self.customer_name):
            frappe.throw(_("Special Character Not Allowed."))
    ...
    def create_lead_address_contact(self):
        if self.lead_name:
            ...
            lead = frappe.db.get_value("Lead", self.lead_name,
                                       ["lead_name", "email_id", "phone", "mobile_no", "gender", "salutation"],
                                       as_dict=True)
    ...
    def validate_credit_limit_on_change(self):
        ...
        for company in frappe.get_all("Company"):
            ...
            if flt(self.credit_limit) < outstanding_amt:
                frappe.throw(_(
                    """New credit limit is less than current outstanding amount for the customer. Credit limit has to be atleast {0}""").format(
                    outstanding_amt))
