...
class ProductBundle(Document):
	...
	def duplicate_product_check(self):
		get_item_code = frappe.db.get_value("Item", self.new_item_code, "item_code")
		for item in self.items:
			if get_item_code == item.item_code:
				frappe.throw(_("Product Bundle can not add as Item {0} ").format(self.new_item_code))
