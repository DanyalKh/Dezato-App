...
def submit_invoice(doc):
    ...
    doc.run_method("calculate_taxrate_taxamount")
    doc.taxes_and_charges = args.get('taxes_and_charges')
...
@frappe.whitelist()
def get_pos_profile_warehouse():
    warehouse = ''
    pos_profile = frappe.get_all('POS Profile', fields=["warehouse"], filters={"user":frappe.session.user})
    for w in pos_profile:
